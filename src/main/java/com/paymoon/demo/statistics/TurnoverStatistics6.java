...
//*
// */
//public class TurnoverStatistics6 {
//
//	/**
//	 * @param args
...
//		new TurnoverStatistics6().connectDB2();
//		MailSend.send();
//	}
//
...
////			String tradeStatisticsSQL =  "select prim.MERID,prim.MERNAME,prim.GOODSID,prim.GOODSNAME,prim.BANKNAME,SUM(prim.AMOUNT) TOTALAMOUNT from (select t.*,bank.BANKNAME,goods.GOODSNAME,mer.MERNAME from UMPAY.T_UPTRANS_" + tableSuf + " as t left join UMPAY.T_BANK_INF as bank on bank.BANKID=t.BANKID left join UMPAY.T_GOODS_INF goods on goods.GOODSID=t.GOODSID and goods.MERID=t.MERID left join UMPAY.T_MER_INF as mer on t.MERID=mer.MERID) prim where prim.TRANSDATE='" + todayyyyMMdd + "' and prim.TRANSTATE=0 group by prim.MERID,prim.MERNAME,prim.GOODSID,prim.GOODSNAME,prim.BANKNAME";
//
...
//
//			//转化率
//			String transRateSQL =  "select prim.MERID,prim.MERNAME,prim.GOODSID,prim.GOODSNAME,prim.BANKID,prim.BANKNAME,COUNT(prim.ORDERSTATE) ORDERCOUNT from (select t.*,bank.BANKNAME,goods.GOODSNAME,mer.MERNAME from UMPAY.T_UPORDER_" + tableSuf + " as t left join UMPAY.T_BANK_INF as bank on bank.BANKID=t.BANKID left join UMPAY.T_GOODS_INF goods on goods.GOODSID=t.GOODSID and goods.MERID=t.MERID left join UMPAY.T_MER_INF as mer on t.MERID=mer.MERID) prim where prim.ORDERDATE='" + todayyyyMMdd + "' group by prim.MERID,prim.MERNAME,prim.GOODSID,prim.GOODSNAME,prim.BANKID,prim.BANKNAME";
//			rs = stmt.executeQuery(transRateSQL);
//			transRate(wwb,stmt2,rs);
//
...
//						sheet.addCell(label);
//				String transRateSQL = "select MERID,GOODSID,BANKID,TRANSTATE,COUNT(TRANSTATE) TRANSCOUNT from UMPAY.T_UPTRANS_" + tableSuf + " where TRANSDATE='" + todayyyyMMdd + "' and MERID='"+merid +"' and GOODSID='"+goodsid+"' and BANKID='"+bankid+"'   group by MERID,GOODSID,BANKID,TRANSTATE";
...
//				int orderSuccess = 0;
//				while(rtrans.next()) {
//					String transState = rtrans.getString("TRANSTATE");
//					int transCountDetail = Integer.parseInt(rtrans.getString("TRANSCOUNT"));
//
//					if (transState.equals("1")) {
//						orderSuccess = transCountDetail;
//					}else if (transState.equals("-1")) {
//						orderFailed = transCountDetail;
//					}
//					if (!transState.equals("0")) {
//						orderPay +=transCountDetail;
//					}
//				}

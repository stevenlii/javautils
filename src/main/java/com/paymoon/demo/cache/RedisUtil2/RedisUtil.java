...
/**
 * <p>redis通用工具类</p>
 * @author Stevenlii@qq.com
 *
 */
public class RedisUtil {
	private static class RedisUtilHolder {
		static String host = MyProp.getVariable("redis.host");
		static String port = MyProp.getVariable("redis.port");
		private static final RedisUtil redisUtilInstance = new RedisUtil(host,Integer.valueOf(port));
	} 
	...
	public static final RedisUtil getInstance() {
		return RedisUtilHolder.redisUtilInstance;
	}
  ...
  public RedisUtil(String ip, int prot) {
	  if (pool == null) {
		  ...
		  // 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
			String  maxIdle = MyProp.getVariable("redis.maxIdle");
		  ...
		  pool = new JedisPool(config, ip, prot, 100000,password);
	  }
  }
  ...
  public  void destroyAll(Jedis jedis) {
	  if (jedis != null) {  
		  jedis.close(); 

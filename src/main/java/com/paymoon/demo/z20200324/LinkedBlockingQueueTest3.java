...
public class LinkedBlockingQueueTest3 {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		int keySize = 0;
		int queueContent2 = 0;
		for (Entry<String, LinkedBlockingQueue<String>> entry : segmentMap.entrySet()) {
			keySize++;
			queueContent2 += entry.getValue().size();
		}
		System.out.println("共分了多少桶：" + keys.size());
		System.out.println("共有多少消息：" + queueContent);
	}
		...
		public void run() {
			try {
				...
				if (segmentMap.containsKey(String.valueOf(randomNum))
					...
					System.out.println(segmentMap.get(String.valueOf(randomNum)).element());
				} else {
					LinkedBlockingQueue<String> messageQueue = new LinkedBlockingQueue<>();
					segmentMap.put(randomNum + "", messageQueue);
				}
